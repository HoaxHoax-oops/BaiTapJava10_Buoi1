...
public class Bai5 {
	...
	private static float tinhDoDai(float[] diemA, float[] diemB) {
		return (float) Math.sqrt(Math.pow(Math.abs(diemA[0]-diemB[0]), 2) + Math.pow(Math.abs(diemA[1]-diemB[1]), 2));
	}

	private static float[] nhapToaDo(Scanner scan, float[] diem, char chr) {
